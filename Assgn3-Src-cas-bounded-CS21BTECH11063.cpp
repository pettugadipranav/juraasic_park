...
std::atomic<bool> flag{false}; //atomic variable which is used for compare and swap
stringstream stream;
random_device rd;
bool *waiting; //waiting[i]=true=> The thread i is waiting
mt19937 gen(rd());
...
void * each_thread_tas(void *arg) // main algorithm for exectution of Bounded CAS ME 
{
    std::string tempo;
    int i,j=0;
    ...
    for (i = 0; i < k; i++)
    {
        ...
        key=false; 
        waiting[t_no]=true;
        while (waiting[t_no] && key ==0)
        {
            key=flag.compare_exchange_weak(expect,true, std::memory_order_acq_rel);
            expect = false;
        } 
        waiting[t_no]=false;
        tempo=get_time();
        ...
        sleeper(t1); //Simulation of critical section
        tempo=get_time();
        exit_t[t_no][i]=tempo;    //exit section
        j = (i + 1) % n;
        while ((j != i) && !waiting[j]) j = (j + 1) % n;
        if (j == i) flag.store(false, std::memory_order_release);
